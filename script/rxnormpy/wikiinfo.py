...
def insertValue(cursor, data):
    if 'link' not in data:
        data['link'] = ''
    sql = 'insert into wiki_rxnorm_info_value (entity_id, property_id, hierarchy, value,link) values ("%s","%s","%s","%s","%s")' % (data['eid'],data['pid'],data['hi'],mysqlformat(data['value']), data['link'])
    cursor.execute(sql)
...
def insertProperty(cursor, propertyName):
    ...
    if pid:
        ...
        sql = 'insert into wiki_rxnorm_info_property (name) values ("%s")' % (propertyName)
        cursor.execute(sql)
        return cursor.lastrowid

def getPropertyID(cursor, propertyName):
    propertyName = pfilter(propertyName)
    sql = 'select id from wiki_rxnorm_info_property where name = "%s"' % (propertyName)
...
def process(db,cursor, entityID, link):
    ...
    for child in mwContent:
        if isinstance(child.tag,basestring):
            ...
            #elif 'p' in '' + child.tag or 'ul' in child.tag:
            #    print child.text_content()[0:10]

def main():

    db = MySQLdb.connect("localhost","root","","rxnorm" )
    
    cursor = db.cursor()
    
    sql = "select id, link from wiki_rxnorm_info_entity"
